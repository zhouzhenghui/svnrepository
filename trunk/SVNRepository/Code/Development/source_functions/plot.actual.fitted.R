...
"plot.actual.fitted"=function(regression.dataframe,state,dates, sreturn=TRUE){
	...
	test.and.plot = dataframe.eda(regression.dataframe, state, dates.temp, basicTS = F, comparisonTS = T, allTS = T, CCF = F, scatter = F, ACF = F, PACF = F)
	test.and.plot
}