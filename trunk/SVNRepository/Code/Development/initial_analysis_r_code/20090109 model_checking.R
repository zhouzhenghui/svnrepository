...
source("../source_functions/forecasting.R")
...
all_states = read.csv("../../../Data/States/states4.csv")
...
for (i in 1:length(as.matrix(state.names))){
	...
	if(state.unit.roots[i]==0){
		...
		#forecasting_vec = c(forecasting_vec,forecasting(current.state.name,132))
	}
