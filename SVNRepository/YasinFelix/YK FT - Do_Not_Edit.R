...
## --Build ARIMA Model
arima.fit = arima(current.data.diff,order = c(3,0,2))
arima.fit
...
garch.fit = garchOxFit(formula.mean=~arma(0,1),formula.var=~garch(1,1),series=current.data.diff)
garch.fit = garchFit(formula = ~arma(1,1)+garch(1,1), data=current.data.diff,cond.dist = c("QMLE"), include.mean=TRUE)
