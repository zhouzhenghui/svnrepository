...
"Box.Ljung.test" = function(data, lag = 12, adj.DF = NULL){
    ...
    adj.p.value =  round(1-pchisq(stat, DF),4)
    Cnames = c(paste("Q(", lag, ")", sep = ""), "P-value", "df")
    if(is.null(adj.DF)){
        temp = as.data.frame(cbind(stat, p.value, DF))
        names(temp) = Cnames
        print(temp, srn = T)
    }
    else{
        temp = as.data.frame(cbind(stat, adj.p.value, DF))
        names(temp) = Cnames
        print(temp, srn = T)
    }
}
